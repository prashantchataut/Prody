...
class OnboardingViewModel @Inject constructor(
    ...
    /**
     * Completes the onboarding process by initializing user data and populating initial content.
     *
     * Performance Optimization:
     * - Uses `database.withTransaction` to group all database insertions into a single transaction.
     *   This drastically reduces disk I/O overhead compared to multiple individual DAO calls.
     * - Keeps DataStore updates (`preferencesManager`) outside the transaction to avoid blocking.
     */
    fun completeOnboarding() {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                // 1. Initial DataStore updates (Non-blocking for DB)
                ...
                // 2. Atomic Database Initialization
                database.withTransaction {
                    ...
                    populateInitialContentInTransaction()
                }

                // 3. Mark onboarding as completed in DataStore after DB success
                preferencesManager.setOnboardingCompleted(true)

            } catch (e: Exception) {
                android.util.Log.e(TAG, "Error during onboarding completion", e)
                // Still mark onboarding as completed to prevent being stuck in a loop if it's just a non-critical error
                try {
                    preferencesManager.setOnboardingCompleted(true)
    ...
    /**
     * Performance Optimization: This helper is designed to run within an existing Room transaction.
     */
    private suspend fun populateInitialContentInTransaction() {
        // Bulk insert all initial content
