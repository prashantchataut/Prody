...
class SecureDatabaseManager @Inject constructor(
    ...
    fun getDatabasePassphraseSync(): String {
        try {
            ...
            if (!success) {
                throw IOException("Failed to save new database passphrase to EncryptedSharedPreferences")
            }
            return newKey
        } catch (e: Exception) {
            Log.e(TAG, "Critical failure getting database passphrase", e)
            throw SecurityException("Database encryption key could not be securely retrieved or generated.", e)
        }
    }
    ...
    /**
     * Generate a secure 256-bit database key using high-entropy SecureRandom.
     */
