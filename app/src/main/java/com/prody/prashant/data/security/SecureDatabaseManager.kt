...
class SecureDatabaseManager @Inject constructor(
    ...
    fun getDatabasePassphraseSync(): String {
        return try {
            ...
            // Security: Throwing an exception here ensures the app fails securely rather than falling back to
            // a weak, deterministic, or hardcoded passphrase.
            Log.e(TAG, "CRITICAL: Error getting database passphrase. Security violation.", e)
            throw SecurityException("Could not securely retrieve or generate database encryption key.", e)
        }
    }
