...
import com.kairos.app.domain.haven.WitnessModeManager
...
class KairosApplication : Application(), Configuration.Provider {
    ...
    override fun onCreate() {
        ...
        if (inCrashProcess) {
            ...
            // and Hilt's Hilt_KairosApplication handles the Hilt setup we want to skip.
    ...
    private fun initializeApp() {
        ...
        if (BuildConfig.DEBUG) {
            applicationScope.launch(Dispatchers.IO) {
                try {
                    val clazz = Class.forName("com.kairos.app.debug.mcp.KairosMcpServer")
                    val instance = clazz.getField("INSTANCE").get(null)
                    val method = clazz.getMethod("start", Context::class.java)
                    method.invoke(instance, this@KairosApplication)
                } catch (e: Exception) {
    ...
    companion object {
        ...
        const val CHANNEL_MAIN = "Kairos_main"
        const val CHANNEL_WISDOM = "Kairos_wisdom"
        const val CHANNEL_JOURNAL = "Kairos_journal"
        const val CHANNEL_FUTURE = "Kairos_future"
        const val CHANNEL_ACHIEVEMENTS = "Kairos_achievements"
        const val CHANNEL_HAVEN = "Kairos_haven"
    }
}
