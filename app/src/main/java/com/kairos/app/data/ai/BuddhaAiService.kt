...
import com.kairos.app.data.cache.AiCacheManager
...
class BuddhaAiService @Inject constructor(
    ...
    suspend fun getDailyWisdom(): GeminiResult<String> = withContext(Dispatchers.IO) {
        ...
        Log.d(TAG, "Cache miss for daily wisdom, generating fresh content")
        // Generate fresh wisdom
        val result = geminiService.generateDailyWisdom()

        // Cache successful results
        if (result is GeminiResult.Success) {
            Log.d(TAG, "Daily wisdom generation successful, caching result")
    ...
    suspend fun getJournalPrompt(mood: Mood): GeminiResult<String> = withContext(Dispatchers.IO) {
        ...
        if (cached != null) {
            Log.d(TAG, "Returning cached journal prompt for mood: $moodKey")
            return@withContext GeminiResult.Success(cached)
        }

        Log.d(TAG, "Cache miss for journal prompt ($moodKey), generating fresh content")
        // Generate fresh prompt
        val result = geminiService.generateJournalPrompt(mood)

        // Cache successful results
        if (result is GeminiResult.Success) {
            Log.d(TAG, "Journal prompt generation successful, caching result")
    ...
    suspend fun invalidateDailyWisdomCache() {
        cacheManager.invalidateCacheType(com.kairos.app.data.cache.AiCacheType.DAILY_WISDOM)
    }
    ...
    suspend fun invalidateWeeklySummaryCache() {
        cacheManager.invalidateCacheType(com.kairos.app.data.cache.AiCacheType.WEEKLY_SUMMARY)
    }
