...
class BackupManager @Inject constructor(
     ...
     * @return KairosBackup object containing all user data
     */
    suspend fun createBackup(): KairosBackup = withContext(Dispatchers.IO) {
        ...
        KairosBackup(
            version = KairosBackup.BACKUP_VERSION,
    ...
    suspend fun exportBackup(): Uri? = withContext(Dispatchers.IO) {
        try {
            ...
            val fileName = "Kairos_backup_$timestamp.json"
    ...
    suspend fun restoreBackup(
        ...
        try {
            ...
            val backup = json.decodeFromString<KairosBackup>(jsonString)

            // Version check
            if (backup.version > KairosBackup.BACKUP_VERSION) {
    ...
    suspend fun cleanupOldBackups(keepCount: Int = 5) = withContext(Dispatchers.IO) {
        val backupDir = File(context.cacheDir, "backups")
        if (backupDir.exists()) {
            val files = backupDir.listFiles { _, name -> name.startsWith("Kairos_backup_") && name.endsWith(".json") }
