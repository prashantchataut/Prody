...
import com.kairos.app.data.moderation.ContentModerationManager
...
class DataHygieneManager @Inject constructor(
    ...
    suspend fun cleanupBackupCache(keepCount: Int = 3): CleanupAction = withContext(Dispatchers.IO) {
        ...
        if (backupDir.exists()) {
            val files = backupDir.listFiles { _, name ->
                name.startsWith("Kairos_backup_") && name.endsWith(".json")
    ...
    suspend fun getStorageStats(): StorageStats = withContext(Dispatchers.IO) {
        ...
        // Database size
        val dbFile = context.getDatabasePath("Kairos_database")
