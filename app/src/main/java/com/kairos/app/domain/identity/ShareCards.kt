 ...
 * - Minimal branding: Subtle Kairos attribution, user is the hero
 ...
 *     title = KairosTitles.findById("philosopher"),
 *     banner = KairosBanners.findById("sages_horizon"),
 *     frame = KairosFrames.findById("contemplative_ring"),
 *     pinnedBadges = listOf(...),
...
object ShareCards {
    ...
    fun getShareText(card: ShareCard): String = when (card) {
        is ProfileCard -> "Check out my journey on Kairos! #Kairos #PersonalGrowth"
        is AchievementCard -> "I just unlocked \"${card.achievementName}\" on Kairos! ${card.celebrationMessage} #Kairos"
        is BannerUnlockCard -> "New banner unlocked: ${card.bannerName}! #Kairos"
        is StreakCard -> "${card.streakMessage}! #Kairos #${card.currentStreak}DayStreak"
        is LevelUpCard -> "Level ${card.newLevel} reached! I'm now a ${card.newLevelName}. #Kairos"
        is RankCard -> "Ranked #${card.rank} this week! #Kairos #Leaderboard"
        is JourneyCard -> "My ${card.periodType.displayName.lowercase()} journey on Kairos: ${card.pointsEarned} points earned! #Kairos"
        else -> "Sharing from Kairos #PersonalGrowth"
    }
     ...
     */
    fun getWatermarkText(): String = "Kairos"
