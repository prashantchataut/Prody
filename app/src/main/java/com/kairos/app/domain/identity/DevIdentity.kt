 ...
 * the highest tier of identity customization in Kairos.
 *
...
object DevIdentity {
    ...
    fun verifyDevCosmetics(): DevCosmeticsVerification {
        val banner = KairosBanners.findById(DEV_BANNER_ID)
        val frame = KairosFrames.findById(DEV_FRAME_ID)
        val title = KairosTitles.findById(DEV_TITLE_ID)
    ...
    object DisplayConfig {
        ...
        val BADGE_SURFACES = setOf(
            KairosBanners.DisplaySurface.PROFILE_HEADER,
            KairosBanners.DisplaySurface.LEADERBOARD_ROW,
            KairosBanners.DisplaySurface.SHARE_CARD,
            KairosBanners.DisplaySurface.MESSAGE_CARD
        )
     ...
     * Green-themed to match Kairos brand.
     */
