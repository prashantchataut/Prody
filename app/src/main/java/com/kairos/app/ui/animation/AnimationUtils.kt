...
/**
 * Kairos Design System - Animation Utilities
 *
...
/**
 * Standard animation transitions for Kairos app.
 * Use these for consistent motion language across the app.
 */
object KairosAnimations {

    // ----- Fade Transitions -----

    val fadeIn: EnterTransition = fadeIn(
        animationSpec = tween(KairosTokens.Animation.normal)
    )
    ...
    val slideInFromBottom: EnterTransition = slideInVertically(
        initialOffsetY = { it / 4 },
        animationSpec = tween(KairosTokens.Animation.normal, easing = EaseOutCubic)
    ) + fadeIn(animationSpec = tween(KairosTokens.Animation.normal))

    val slideOutToBottom: ExitTransition = slideOutVertically(
        targetOffsetY = { it / 4 },
        animationSpec = tween(KairosTokens.Animation.normal, easing = EaseInCubic)
    ) + fadeOut(animationSpec = tween(KairosTokens.Animation.normal))
    ...
    val scaleOut: ExitTransition = scaleOut(
        targetScale = 0.9f,
        animationSpec = tween(KairosTokens.Animation.fast)
    ) + fadeOut(animationSpec = tween(KairosTokens.Animation.fast))
    ...
    val collapseOut: ExitTransition = scaleOut(
        targetScale = 0.8f,
        animationSpec = tween(KairosTokens.Animation.fast, easing = EaseInCubic)
    ) + fadeOut(animationSpec = tween(KairosTokens.Animation.fast))
    ...
    ) + fadeIn(animationSpec = tween(KairosTokens.Animation.fast))
 ...
 */
fun staggeredDelay(index: Int, baseDelay: Int = KairosTokens.Animation.listStagger): Int {
...
fun staggeredAnimationStates(
    itemCount: Int,
    delayPerItem: Int = KairosTokens.Animation.listStagger
): List<State<Float>> {
...
fun Modifier.breathingEffect(
    ...
    duration: Int = KairosTokens.Animation.glowPulse
): Modifier = composed {
...
object KairosAnimationSpecs {
    ...
    val microInteraction = tween<Float>(
        durationMillis = KairosTokens.Animation.fast,
        easing = EaseOutCubic
    )
    ...
    val standardTransition = tween<Float>(
        durationMillis = KairosTokens.Animation.normal,
    ...
    val emphasis = tween<Float>(
        durationMillis = KairosTokens.Animation.slow,
        easing = EaseOutCubic
    )
}
...
object CelebrationAnimations {
     ...
     */
    const val achievementRevealDuration = KairosTokens.Animation.achievementReveal

    /**
     * Duration for celebration burst effect
     */
    const val celebrationBurstDuration = KairosTokens.Animation.celebrationBurst
