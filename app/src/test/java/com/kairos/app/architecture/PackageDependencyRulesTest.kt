package com.kairos.app.architecture
...
class PackageDependencyRulesTest {
    ...
    private val sourceRoot = File("src/main/java/com/Kairos/prashant")
    ...
    fun `domain layer does not depend on data or ui layers`() {
        ...
        val violations = kotlinFilesIn("domain")
            ...
            .flatMap { file ->
                forbiddenImports(file, listOf("com.kairos.app.data", "com.kairos.app.ui"))
            }
    ...
    fun `data layer does not depend on ui layer`() {
        val violations = kotlinFilesIn("data")
            .flatMap { file ->
                forbiddenImports(file, listOf("com.kairos.app.ui"))
            }
    ...
    fun `data layer must not depend on domain layer`() {
        val violations = kotlinFilesIn("data")
            .flatMap { file ->
                forbiddenImports(file, listOf("com.kairos.app.domain"))
            }
    ...
    fun `ui layer must not depend on data layer directly`() {
        val allowedDataImports = listOf(
            "com.kairos.app.data.local.preferences",
            "com.kairos.app.data.network"
        )
            ...
            .flatMap { file ->
                file.readLines()
                    .mapIndexedNotNull { index, line ->
                        ...
                        if (trimmed.startsWith("import com.kairos.app.data.") &&
