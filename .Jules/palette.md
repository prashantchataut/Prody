...
## 2025-01-26 - Standardized Icon Button Tooltips
**Learning:** Standardizing icon buttons with integrated tooltips improves both accessibility and discoverability without cluttering the UI. Using the tooltip as an automatic fallback for content description ensures that developers only need to provide one string for both visual and non-visual users.
**Action:** For all future icon-only interactions, use components that support integrated tooltips (like the updated `ProdyIconButton`) to maintain consistency and meet WCAG standards for interactive elements.
