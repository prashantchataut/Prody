...
## 2026-02-05 - Centralized Privacy Protection and Expansion
**Vulnerability:** Inconsistent and missing screenshot/recording protection across multiple sensitive features (Locker, Future Messages, Collaborative Messaging, Monthly Letters).
**Learning:** Decentralized security logic leads to coverage gaps. Sensitive features added later often missed the manual `FLAG_SECURE` implementation used in older screens.
**Prevention:** Use a centralized `SecureScreen` Composable and establish a "Secure by Default" policy for any screen handling private user data.
